...
object Solution12 extends App {
  def parseInput(is: InputStream): Option[List[(String, String)]] = Try {
    ...
    val lines = Iterator.continually(r.readLine()).takeWhile(_ != null).drop(1).
      toList.zipWithIndex
    val (evens, odds) = lines.partition(_._2 % 2 == 0)
    evens.map(_._1) zip odds.map(_._1)
  }.toOption

  def substrings(s: String, l: Int): TraversableOnce[String] = Stream.from(0).
    takeWhile(_ <= s.length - l)
    .map(i => s.drop(i).take(l))

  def crossProduct[A, B](a: TraversableOnce[A], b: TraversableOnce[B]):
  TraversableOnce[(A, B)] = for { i <- a; j <- b } yield (i, j)
